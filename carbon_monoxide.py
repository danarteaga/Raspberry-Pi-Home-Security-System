...
# This script will send a notification if the carbon monoxide alarm is set off
...
if send_text == True:
	for my_number in text_list:		
		to_send1 = 'echo "%s" | mail -s "Carbon Monoxide Detected!" %s' %(my_camera,my_number)
		os.system(to_send1)	
		
if send_email_basic == True:
	for my_email in email_list:			
		to_send1 = 'echo "%s" | mail -s "Carbon Monoxide Detected!" %s' %(my_camera,my_email)
		os.system(to_send1)			

if use_pushbullet == True:	
	for itir,pushme1 in enumerate(pushbullet_list):	
		to_send1 = '%s' %(my_camera)
		pushme1.push_link("Carbon Monoxide Detected!",to_send1)	
